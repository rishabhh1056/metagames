...
import static com.metagards.metagames._RummyPull.RummyPullGame.removeLastChars;
...
public class GameLocalLogic {
    ...
    private boolean isColorMatch() {
        ...
        Log.e("CardColorArray_", Arrays.toString(CardColorArray));
        boolean isColorMatch = false;
        outerloop:
        for (int i = 0; i < CardColorArray.length; i++) {
        ...
        Log.e("ismatched_", String.valueOf(isColorMatch));
    ...
    private boolean straightCard(){
        ...
        Log.e("cardNumberArray", Arrays.toString(cardNumberArray));
        ...
        if (!isStraight){
            if(in_array(14,cardNumberArray))
            {
                cardNumberList.remove(Arrays.asList(Integer.valueOf(-1)));
                cardNumberArray = cardNumberList.toArray(new Integer[cardNumberList.size()]);
                cardNumberArray = str_replace(14,1,cardNumberArray);
                Arrays.sort(cardNumberArray);

                isStraight = checkSequence(cardNumberArray,0);
                Log.d("is_Straight_not_true",Arrays.toString(cardNumberArray));
                Log.d("is_Straignt_false", String.valueOf(isStraight));

            }
        }

        Log.e("isStraignt_", String.valueOf(isStraight));
        return isStraight;
    }
