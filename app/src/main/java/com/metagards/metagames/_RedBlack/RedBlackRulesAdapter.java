...
public class RedBlackRulesAdapter extends RecyclerView.Adapter<RedBlackRulesAdapter.holder> {

    List<JackpotRulesModel> arraylist;
    OnItemClickListener callback;
    Context context;

    public RedBlackRulesAdapter(Context context) {
        this.context = context;
    }

    @Override
    public int getItemViewType(int position) {
        return position > 2 ? JackpotRulesModel.TYPE_HIGH : JackpotRulesModel.TYPE_LOW;
    }
    ...
    public holder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = null;
        if (viewType==JackpotRulesModel.TYPE_HIGH) {
            view = LayoutInflater.from(context).inflate(R.layout.item_redblack_setamount, parent, false);
        } else {
            view = LayoutInflater.from(context).inflate(R.layout.item_redblack_setamount, parent, false);
        }
    ...
    class holder extends RecyclerView.ViewHolder{
        ...
        void bind(JackpotRulesModel model){
            ...
            int item_background = R.drawable.ic_jackpot_rule_bg;
            if(postion == 0)
                item_background = R.drawable.ic_round_cornerbox_red;
            if(postion == 1)
                item_background = R.drawable.ic_round_cornerbox_black;

            ivContainerbg.setBackground(Functions.getDrawable(context,item_background));
            ...
            itemView.findViewById(R.id.rltContainer).setOnClickListener(new View.OnClickListener() {
               ...
               //     ((RedBlackPot_A)context).playButtonTouchSound();
                    ((RedBlackPot_Socket)context).playButtonTouchSound();
    ...
    private void initiAnimation() {
        ...
        blinksAnimation.setDuration(1500);
