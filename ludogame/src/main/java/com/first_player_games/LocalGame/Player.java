    ...
    public void move(int piece,int steps){
        if(!moving[piece])
            if(p[piece] == 0 && steps == 6)
                animateMovement(piece,1,false);
            else if(p[piece] > 0 ) {
                if(p[piece]+steps<57)
                    animateMovement(piece, steps,false);
                else if(p[piece]+steps==57)
                    animateMovement(piece, steps,true);
            }

    }

    public void animateMovement(final int piece, final int steps, final boolean last){
        moving[piece] = true;
        ...
        a1.addListener(new Animator.AnimatorListener() {
            ...
            @Override public void onAnimationEnd(Animator animator) {
                ...
                if(steps>1)
                    animateMovement(piece,steps -1,last);
                else {
                    moving[piece]  = false;
